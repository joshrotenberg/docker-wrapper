...
impl StopCommand {
    /// Create a new stop command for the specified container(s)
    ///
    /// # Examples
    ///
    /// ```
    ...
    ///
    /// let cmd = StopCommand::new("my-container");
    /// ```
    ///
    /// ```
    /// use docker_wrapper::StopCommand;
    ...
    /// Create a new stop command for multiple containers
    ///
    /// # Examples
    ///
    /// ```
...
impl DockerCommand for StopCommand {
    ...
    async fn execute(&self) -> Result<Self::Output> {
        ...
        let stopped_containers = if output.stdout.trim().is_empty() {
            // If no stdout, assume the containers specified were stopped
